...
export default function LoginPage(){
    ...
    if (!isChecked) return;
    router.push('/login?redirect=/workspace/make');
    };

    const handleRegisterCardClick = () => {
        if (!isChecked) return;
        router.push('/register/president/step1?redirect=/workspace/make');
      };
  ...
  className="self-stretch h-64 px-12 py-24 bg-white rounded-xl outline outline-1 outline-offset-[-1px] outline-zinc-300 inline-flex justify-center items-center gap-2 cursor-pointer"
  onClick={handleLoginCardClick}
>
  <div className={`text-center justify-start text-2xl leading-loose ${isChecked ? 'text-[#5A5D61] font-semibold' : 'text-zinc-300 font-normal'}`}>
    로그인 후 <br />워크스페이스 생성하기
  </div>
</div>
                </div>
                <div className="flex flex-col">

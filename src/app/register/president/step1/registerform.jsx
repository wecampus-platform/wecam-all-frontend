...
function RegisterForm({ onSubmit }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onSubmit({
            school: selectedSchool?.name || schoolInput,
            college: selectedCollege?.name || collegeInput,
            department: selectedDepartment?.name || departmentInput,
            selschool: selectedSchool?.id ?? null,
            selcollege: selectedCollege?.id ?? null,
            seldepartment: selectedDepartment?.id ?? null,
    ...
    return (
        <form onSubmit={handleSubmit} className="w-full bg-white h-[1080px] overflow-hidden text-center text-5xl text-darkslategray font-pretendard relative">
            <div className="absolute top-[160px] left-1/2 -translate-x-1/2 font-semibold">대표자 회원가입</div>
            ...
            <div className="absolute top-[253px] left-1/2 -translate-x-1/2 w-[656px] flex flex-col items-center gap-7 text-left text-base text-silver">
                <div className="self-stretch flex flex-col items-center gap-5">
                    ...
                    <InputWithStyledList
                        ...
                        disabled={!selectedCollege && !collegeInput.trim()}
                    />
